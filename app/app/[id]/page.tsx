...
async function getApp(id: string): Promise<App | null> {
  try {
    ...
    const { data, error } = await supabase
      .from("apps").select("*").eq("id", id).single();
          ...
          <div className="p-8">
            <div className="flex items-start gap-6 mb-6">
              ...
              <div className="flex-1 min-w-0 pt-1">
                ...
                <a href={app.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-blue-400 transition-colors truncate max-w-full">
            ...
            <div className="flex items-center gap-1.5 text-xs text-gray-600">
              <Calendar className="h-3.5 w-3.5" />Added {formattedDate}
            </div>
          </div>
        </div>
